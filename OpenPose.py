...
class OpenPose:
    ...
    #   helper method to create the model and load the weights.
    ...
    #   helper method to calculate and return the heatmap of the given img.
    def _heatmap(self, img):
        ############################### Setup the heatmap vars. ########################
        param, model_params = config_reader()
        multiplier = [x * model_params['boxsize'] / img.shape[0] for x in param['scale_search']]
    ...
    #   MAIN METHOD : return the img's keypoints as an array.
    def keypoints(self, img):
        heatmap_avg, paf_avg = self._heatmap(img)
        param, model_params = config_reader()
        ...
        subset = np.delete(subset, deleteIdx, axis=0)
         ...
         # DONE? """ !!!!!!!!!! TODO: Alter this so that any skipped keypoints are
         # considered a zero value !!!!!!!!!! """

        return_peaks = []

        for arr in all_peaks:
            try:
                if len(arr[0]) != 0:
                    return_peaks.append((arr[0][0], arr[0][1]))
            except:
                return_peaks.append((0, 0))
